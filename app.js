...
function saveReceipt(){
  ...
  const{year,month}=dateToYM(date);
  const d=loadFor(b,year,month);
  const receiptRecord={id:Date.now(),no,date,invNo:document.getElementById('r-inv-no').value.trim()||document.getElementById('r-inv-ref').value,salesPerson:document.getElementById('r-sales').value.trim(),customer:cust,items,saleTotal:st,costTotal:ct,commMode,commRate:cr,commAmt:comm,profit:st-ct-comm,note:document.getElementById('r-note').value.trim()};
  ...
  resetF('receipt');onYearChange();renderDash();alert('บันทึกใบเสร็จรับเงินเรียบร้อย!');
}
...
function resetF(t){
  ...
  if(t==='receipt'){['r-no','r-cust','r-sales','r-inv-no','r-cr','r-note'].forEach(id=>document.getElementById(id).value='');document.getElementById('r-comm-mode').value='percent';toggleCommMode('r');document.getElementById('r-date').value=todayStr;document.getElementById('r-items-body').innerHTML='';['r-st','r-ct','r-ca','r-pf'].forEach(id=>document.getElementById(id).value='');}
