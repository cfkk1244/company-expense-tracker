...
function dashMetricLabel(metric){return{sales:'ยอดขายจากบิลใบส่งสินค้า',production:'ยอดสั่งผลิตสินค้า',profit:'กำไรสุทธิ',expense:'ค่าใช้จ่าย'}[metric]||metric;}
function dashMetricClass(metric){return{sales:'',production:'purple',profit:'green',expense:'red'}[metric]||'';}
function metricFromData(d,metric){
  const invoices=d.invoices||[], expenses=d.expenses||[], productions=d.productions||[];
  if(metric==='sales')return invoices.reduce((s,x)=>s+safeNum(x.saleTotal),0);
  if(metric==='production')return productions.reduce((s,x)=>s+safeNum(x.total||x.subtotal),0);
...
function rowsForMonthlyChart(year,branches,metric){
  return MONTHS.map((name,m)=>{
    let value=0;
    branches.forEach(br=>{value+=metricFromData(loadFor(br,year,m),metric);});
    return{label:name,value,sub:`${name} ${year+543}`};
  });
}
function rowsForYearlyChart(branches,metric){
  ...
  return years.map(year=>{
    let value=0;
    branches.forEach(br=>{for(let m=0;m<12;m++)value+=metricFromData(loadFor(br,year,m),metric);});
    return{label:String(year+543),value,sub:`ปี ${year} (${year+543})`};
  });
}
...
function customerRows(year,monthVal,branches,source,mode){
  ...
  branches.forEach(br=>months.forEach(m=>{
    ...
    if(source==='all'||source==='invoices'){
      (d.invoices||[]).forEach(x=>add(x.customer,safeNum(x.saleTotal),'invoice'));
    }
    if(source==='all'||source==='productions'){
      (d.productions||[]).forEach(x=>add(x.customer,safeNum(x.total||x.subtotal),'production'));
    }
  }));
...
function renderCustomerChart(){
  ...
  if(summary)summary.innerHTML=`แสดงลูกค้า Top 10 จาก <b>${srcLabel}</b> ช่วง ${monthText} ${year+543}<br>${mode==='count'?'จำนวนเอกสารรวม':'มูลค่ารวมในกราฟ'}: <b>${mode==='count'?chartCount(total):chartMoney(total)}</b>`;
}
...
function calcI(){let st=0,ct=0;document.querySelectorAll('#i-items-body tr').forEach(tr=>{const ins=tr.querySelectorAll('input[type=number]');const qty=parseFloat(ins[0]?.value)||0,cu=parseFloat(ins[1]?.value)||0,pu=parseFloat(ins[2]?.value)||0,s=qty*pu,c=qty*cu;st+=s;ct+=c;const ros=tr.querySelectorAll('input.ro');if(ros[0])ros[0].value=s?fmt(s):'';if(ros[1])ros[1].value=c?fmt(c):'';});const cr=parseFloat(document.getElementById('i-cr').value)||0;const mode=getCommMode('i');let comm=mode==='manual'?parseMoney(document.getElementById('i-ca').value):st*cr/100;const pf=st-ct-comm;document.getElementById('i-st').value=st?fmt(st):'';document.getElementById('i-ct').value=ct?fmt(ct):'';if(mode!=='manual')document.getElementById('i-ca').value=comm?fmt(comm):'';document.getElementById('i-pf').value=(st||ct||comm)?fmt(pf):'';document.getElementById('i-pf').style.color=pf<0?'var(--red)':'var(--green)';}
function calcR(){let st=0,ct=0;document.querySelectorAll('#r-items-body tr').forEach(tr=>{const ins=tr.querySelectorAll('input[type=number]');const qty=parseFloat(ins[0]?.value)||0,pu=parseFloat(ins[1]?.value)||0,cu=parseFloat(ins[2]?.value)||0,s=qty*pu,c=qty*cu;st+=s;ct+=c;const ro=tr.querySelector('input.ro');if(ro)ro.value=s?fmt(s):'';});const cr=parseFloat(document.getElementById('r-cr').value)||0;const mode=getCommMode('r');let comm=mode==='manual'?parseMoney(document.getElementById('r-ca').value):st*cr/100;const pf=st-ct-comm;document.getElementById('r-st').value=st?fmt(st):'';document.getElementById('r-ct').value=ct?fmt(ct):'';if(mode!=='manual')document.getElementById('r-ca').value=comm?fmt(comm):'';document.getElementById('r-pf').value=(st||ct||comm)?fmt(pf):'';document.getElementById('r-pf').style.color=pf<0?'var(--red)':'var(--green)';}
...
function saveQuote(){
  ...
  const quoteRecord={id:Date.now(),no,date,customer:cust,salesPerson:document.getElementById('q-sales').value.trim(),items,subtotal:sub,useVat:uv,vatAmt:va,total:sub+va,note:document.getElementById('q-note').value.trim(),attachments:attachedFiles['q-att']||[],approved:false};
  d.quotes.push(quoteRecord);
  saveFor(b,year,month,d);
  if(window.FirebaseService?.saveQuote){window.FirebaseService.saveQuote({...quoteRecord,branch:b,year,month}).catch(err=>console.error('Firebase saveQuote error:',err));}
  attachedFiles['q-att']=[];renderPrev('q-att');resetF('quote');onYearChange();renderDash();alert('บันทึกใบเสนอราคาเรียบร้อย!');
}
...
function saveInvoice(){
  ...
  const cr=parseFloat(document.getElementById('i-cr').value)||0,commMode=getCommMode('i'),comm=commMode==='manual'?parseMoney(document.getElementById('i-ca').value):st*cr/100;
  const{year,month}=dateToYM(date);
  const d=loadFor(b,year,month);
  const invoiceRecord={id:Date.now(),no,date,customer:cust,salesPerson:document.getElementById('i-sales').value.trim(),items,saleTotal:st,costTotal:ct,commMode,commRate:cr,commAmt:comm,profit:st-ct-comm,note:document.getElementById('i-note').value.trim(),attachments:attachedFiles['i-att']||[]};
  d.invoices.push(invoiceRecord);
  saveFor(b,year,month,d);
  if(window.FirebaseService?.saveInvoice){window.FirebaseService.saveInvoice({...invoiceRecord,branch:b,year,month}).catch(err=>console.error('Firebase saveInvoice error:',err));}
  attachedFiles['i-att']=[];renderPrev('i-att');resetF('invoice');onYearChange();renderDash();alert('บันทึกบิลใบส่งสินค้าเรียบร้อย!');
}
...
function saveReceipt(){
  ...
  const cr=parseFloat(document.getElementById('r-cr').value)||0,commMode=getCommMode('r'),comm=commMode==='manual'?parseMoney(document.getElementById('r-ca').value):st*cr/100;
  const{year,month}=dateToYM(date);
  const d=loadFor(b,year,month);
  const receiptRecord={id:Date.now(),no,date,invNo:document.getElementById('r-inv-no').value.trim()||document.getElementById('r-inv-ref').value,salesPerson:document.getElementById('r-sales').value.trim(),customer:cust,items,saleTotal:st,costTotal:ct,commMode,commRate:cr,commAmt:comm,profit:st-ct-comm,note:document.getElementById('r-note').value.trim(),attachments:attachedFiles['r-att']||[]};
  d.receipts.push(receiptRecord);
  saveFor(b,year,month,d);
  if(window.FirebaseService?.saveReceipt){window.FirebaseService.saveReceipt({...receiptRecord,branch:b,year,month}).catch(err=>console.error('Firebase saveReceipt error:',err));}
  attachedFiles['r-att']=[];renderPrev('r-att');resetF('receipt');onYearChange();renderDash();alert('บันทึกใบเสร็จรับเงินเรียบร้อย!');
}
...
function saveExpense(){
  ...
  const{year,month}=dateToYM(date);
  const d=loadFor(b,year,month);
  const expenseRecord={id:Date.now(),date,cat:document.getElementById('e-cat').value,desc,amount,by:document.getElementById('e-by').value.trim(),note:document.getElementById('e-note').value.trim()};
  d.expenses.push(expenseRecord);
  saveFor(b,year,month,d);
  if(window.FirebaseService?.saveExpense){window.FirebaseService.saveExpense({...expenseRecord,branch:b,year,month}).catch(err=>console.error('Firebase saveExpense error:',err));}
  resetF('expense');onYearChange();renderDash();alert('บันทึกค่าใช้จ่ายเรียบร้อย!');
}
...
function resetF(t){
  ...
  if(t==='invoice'){['i-no','i-cust','i-sales','i-cr','i-note'].forEach(id=>document.getElementById(id).value='');document.getElementById('i-comm-mode').value='percent';toggleCommMode('i');document.getElementById('i-date').value=todayStr;document.getElementById('i-items-body').innerHTML='';['i-st','i-ct','i-ca','i-pf'].forEach(id=>document.getElementById(id).value='');attachedFiles['i-att']=[];renderPrev('i-att');}
  if(t==='receipt'){['r-no','r-cust','r-sales','r-inv-no','r-cr','r-note'].forEach(id=>document.getElementById(id).value='');document.getElementById('r-comm-mode').value='percent';toggleCommMode('r');document.getElementById('r-date').value=todayStr;document.getElementById('r-items-body').innerHTML='';['r-st','r-ct','r-ca','r-pf'].forEach(id=>document.getElementById(id).value='');attachedFiles['r-att']=[];renderPrev('r-att');}
...
function fillFromInv(){
  ...
  document.getElementById('r-comm-mode').value=inv.commMode||'percent';
  document.getElementById('r-cr').value=inv.commRate||'';
  document.getElementById('r-ca').value=inv.commMode==='manual'?(inv.commAmt||''):'';
  toggleCommMode('r');
...
function exportXLSX(type,options={}){
  loadSheetJS(()=>{
    ...
    if(type==='all'||type==='invoices'){
      const rows=[['สาขา','ปี','เดือน','เลขที่บิล','วันที่','ลูกค้า','พนักงานขาย','สินค้า','จำนวน','หน่วย','ราคาขาย/หน่วย','ยอดขายรวม','ต้นทุน/หน่วย','ต้นทุนรวม','วิธีคิดคอม','%คอม','คอมมิสชัน','กำไรสุทธิ','หมายเหตุ']];
      years.forEach(y=>branches.forEach(br=>months.forEach(m=>{
        const d=normalizeDataPack(loadFor(br,y,m));
        d.invoices.forEach(inv=>{
          if((inv.items||[]).length){
            (inv.items||[]).forEach((it,idx)=>rows.push([idx===0?BRANCH_TH[br]:'',idx===0?y:'',idx===0?MONTHS[m]:'',idx===0?inv.no:'',idx===0?inv.date:'',idx===0?inv.customer:'',idx===0?inv.salesPerson||'':'',it.product,it.qty,it.unit||'',it.priceUnit,it.saleTotal,it.costUnit||0,it.costTotal,idx===0?(inv.commMode==='manual'?'กรอกเอง':'เปอร์เซ็นต์'):'',idx===0?inv.commRate:'',idx===0?inv.commAmt:'',idx===0?inv.profit:'',idx===0?inv.note||'':'']));
          }else{
            rows.push([BRANCH_TH[br],y,MONTHS[m],inv.no,inv.date,inv.customer,inv.salesPerson||'','',0,'',0,inv.saleTotal,0,inv.costTotal,(inv.commMode==='manual'?'กรอกเอง':'เปอร์เซ็นต์'),inv.commRate,inv.commAmt,inv.profit,inv.note||'']);
          }
        });
      })));
    ...
    if(type==='all'||type==='receipts'){
      const rows=[['สาขา','ปี','เดือน','เลขที่ใบเสร็จ','วันที่','เลขบิล','ลูกค้า','พนักงานขาย','สินค้า','จำนวน','หน่วย','ราคาขาย/หน่วย','ยอดขาย','วิธีคิดคอม','%คอม','คอมมิสชัน','กำไรสุทธิ','หมายเหตุ']];
      years.forEach(y=>branches.forEach(br=>months.forEach(m=>{
        const d=normalizeDataPack(loadFor(br,y,m));
        d.receipts.forEach(r=>{
          if((r.items||[]).length){
            (r.items||[]).forEach((it,idx)=>rows.push([idx===0?BRANCH_TH[br]:'',idx===0?y:'',idx===0?MONTHS[m]:'',idx===0?r.no:'',idx===0?r.date:'',idx===0?r.invNo||'':'',idx===0?r.customer:'',idx===0?r.salesPerson||'':'',it.product,it.qty,it.unit||'',it.priceUnit,it.saleTotal,idx===0?(r.commMode==='manual'?'กรอกเอง':'เปอร์เซ็นต์'):'',idx===0?r.commRate:'',idx===0?r.commAmt:'',idx===0?r.profit:'',idx===0?r.note||'':'']));
          }else{
            rows.push([BRANCH_TH[br],y,MONTHS[m],r.no,r.date,r.invNo||'',r.customer,r.salesPerson||'','',0,'',0,r.saleTotal,(r.commMode==='manual'?'กรอกเอง':'เปอร์เซ็นต์'),r.commRate,r.commAmt,r.profit,r.note||'']);
          }
        });
      })));
